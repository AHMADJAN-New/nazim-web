...
export const useResidencyTypes = (organizationId?: string) => {
  ...
  return useQuery({
    queryKey: ['residency-types', organizationId || profile?.organization_id],
    queryFn: async () => {
      ...
      if (isSuperAdmin) {
        ...
        // Otherwise show all (including global types with organization_id = NULL)
      } else {
        // Regular users see: global types (organization_id = NULL) + their organization's types
        const userOrgId = profile.organization_id;
        if (userOrgId) {
          query = query.or(`organization_id.is.null,organization_id.eq.${userOrgId}`);
        } else {
          // No organization assigned, only show global types
...
export const useCreateResidencyType = () => {
  ...
  return useMutation({
    mutationFn: async (residencyTypeData: { 
      ...
      let organizationId = residencyTypeData.organization_id;
      if (organizationId === undefined) {
        if (profile.role === 'super_admin') {
          // Super admin can create global types (NULL) or org-specific
