...
export const useSubjects = (organizationId?: string) => {
    ...
    return useQuery({
        queryKey: ['subjects', organizationId || profile?.organization_id],
        queryFn: async () => {
            ...
            if (isSuperAdmin) {
                ...
                // Otherwise show all (including global subjects with organization_id = NULL)
            } else {
                // Regular users see: global subjects (organization_id = NULL) + their organization's subjects
                const userOrgId = profile.organization_id;
                if (userOrgId) {
                    query = query.or(`organization_id.is.null,organization_id.eq.${userOrgId}`);
                } else {
                    // No organization assigned, only show global subjects
...
export const useClassSubjects = (classAcademicYearId?: string, organizationId?: string) => {
    ...
    return useQuery({
        queryKey: ['class-subjects', classAcademicYearId, organizationId || profile?.organization_id],
        queryFn: async () => {
            if (!user || !profile || !classAcademicYearId) return [];
...
export const useClassSubjectTemplates = (classId?: string, organizationId?: string) => {
    ...
    return useQuery({
        queryKey: ['class-subject-templates', classId, organizationId],
        queryFn: async () => {
            if (!user || !profile) return [];
