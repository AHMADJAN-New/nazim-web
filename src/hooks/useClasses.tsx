    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (isSuperAdmin) {
                ...
                // Otherwise show all (including global classes with organization_id = NULL)
            } else {
                // Regular users see: global classes (organization_id = NULL) + their organization's classes
                const userOrgId = profile.organization_id;
                if (userOrgId) {
                    query = query.or(`organization_id.is.null,organization_id.eq.${userOrgId}`);
                } else {
                    // No organization assigned, only show global classes
...
export const useClassAcademicYears = (academicYearId?: string, organizationId?: string) => {
    ...
    return useQuery({
        ...
        `);
            ...
            if (!isSuperAdmin) {
                // Regular users see only their organization's class instances
