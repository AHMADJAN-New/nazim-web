...
export const useAcademicYears = (organizationId?: string) => {
  ...
  return useQuery({
    queryKey: ['academic-years', organizationId || profile?.organization_id],
    queryFn: async () => {
      ...
      if (isSuperAdmin) {
        ...
        // Otherwise show all (including global years with organization_id = NULL)
      } else {
        // Regular users see: global years (organization_id = NULL) + their organization's years
        const userOrgId = profile.organization_id;
        if (userOrgId) {
          query = query.or(`organization_id.is.null,organization_id.eq.${userOrgId}`);
        } else {
          // No organization assigned, only show global years
...
export const useCurrentAcademicYear = (organizationId?: string) => {
  ...
  return useQuery({
    queryKey: ['current-academic-year', organizationId || profile?.organization_id],
    queryFn: async () => {
      ...
      if (isSuperAdmin) {
        if (organizationId) {
          ...
          // Get global current year
          query = query.is('organization_id', null);
        }
      } else {
        // Regular users see: global current year or their organization's current year
        const userOrgId = profile.organization_id;
        if (userOrgId) {
          query = query.or(`organization_id.is.null,organization_id.eq.${userOrgId}`);
        } else {
