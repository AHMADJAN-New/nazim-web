...
('school', 'name', '"Nazim Islamic School"', 'School name', true, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('school', 'address', '"Street 45, Block C, Model Town, Lahore"', 'School address', true, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('school', 'phone', '"+92-42-12345678"', 'School phone number', true, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('school', 'email', '"info@nazimschool.edu.pk"', 'School email address', true, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('system', 'timezone', '"Asia/Karachi"', 'System timezone', true, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('system', 'currency', '"PKR"', 'System currency', true, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('system', 'date_format', '"DD/MM/YYYY"', 'Date display format', true, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('academic', 'current_year', '"2024-2025"', 'Current academic year', true, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('notifications', 'email_enabled', 'true', 'Enable email notifications', false, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('notifications', 'sms_enabled', 'false', 'Enable SMS notifications', false, (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1));

-- Insert default report templates
INSERT INTO public.report_templates (name, description, category, type, query_template, parameters, access_level, created_by) VALUES
('Student Attendance Report', 'Daily, weekly, and monthly attendance statistics', 'Attendance', 'Standard', 'SELECT * FROM attendance WHERE date BETWEEN $start_date AND $end_date', '["start_date", "end_date", "class_id"]', 'Staff', (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('Fee Collection Summary', 'Monthly fee collection status and trends', 'Finance', 'Financial', 'SELECT * FROM fees WHERE due_date BETWEEN $start_date AND $end_date', '["start_date", "end_date", "class_id"]', 'Finance', (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('Academic Performance Analysis', 'Student exam results and performance trends', 'Academics', 'Academic', 'SELECT * FROM omr_scans WHERE scanned_at BETWEEN $start_date AND $end_date', '["start_date", "end_date", "exam_id"]', 'Academic', (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('Library Usage Report', 'Book circulation and member activity', 'Library', 'Operational', 'SELECT * FROM library_transactions WHERE issue_date BETWEEN $start_date AND $end_date', '["start_date", "end_date"]', 'Staff', (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1)),
('Hostel Occupancy Report', 'Room occupancy rates and facility utilization', 'Hostel', 'Operational', 'SELECT * FROM hostel_allocations WHERE allocated_date BETWEEN $start_date AND $end_date', '["start_date", "end_date"]', 'Management', (SELECT id FROM public.profiles WHERE role = 'super_admin' LIMIT 1));