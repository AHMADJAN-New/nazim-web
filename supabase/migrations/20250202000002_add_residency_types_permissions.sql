-- ============================================================================
-- Add Residency Types Permissions
-- ============================================================================
-- Add permissions for residency types management and assign to roles
-- ============================================================================

-- Insert residency types permissions (global = organization_id NULL)
INSERT INTO public.permissions (name, resource, action, description, organization_id)
VALUES
    ('academic.residency_types.read', 'academic', 'read', 'View residency types', NULL),
    ('academic.residency_types.create', 'academic', 'create', 'Create residency types', NULL),
    ('academic.residency_types.update', 'academic', 'update', 'Update residency types', NULL),
    ('academic.residency_types.delete', 'academic', 'delete', 'Delete residency types', NULL)
ON CONFLICT (name, organization_id) DO NOTHING;

-- Give super_admin global (organization_id NULL) access to all residency types permissions
INSERT INTO public.role_permissions (role, permission_id, organization_id)
SELECT 'super_admin', id, NULL
FROM public.permissions
WHERE name LIKE 'academic.residency_types.%'
  AND organization_id IS NULL
ON CONFLICT (role, permission_id, organization_id) DO NOTHING;

-- Update assign_default_role_permissions function to include residency types permissions
...
-- Assign residency types permissions to existing organizations' admin roles
DO $$
DECLARE
    org_record RECORD;
BEGIN
    ...
    LOOP
        -- Assign residency types permissions to admin role for this organization
        INSERT INTO public.role_permissions (role, permission_id, organization_id)
        ...
        WHERE p.organization_id IS NULL
          AND p.name = 'academic.residency_types.read'
        ON CONFLICT DO NOTHING;
    END LOOP;
END;
$$;
