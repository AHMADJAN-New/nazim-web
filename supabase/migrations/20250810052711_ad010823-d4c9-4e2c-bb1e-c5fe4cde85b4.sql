...
-- Keep SELECT policy as-is (already present for all users); if needed, we could tighten later

-- 3) Finance tables: tighten SELECT from "all authenticated" to privileged roles
-- donations
DO $$ BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_policies
...
TO authenticated
USING (
  get_user_role(auth.uid()) = ANY (ARRAY['super_admin'::user_role, 'admin'::user_role, 'accountant'::user_role, 'staff'::user_role])
);
