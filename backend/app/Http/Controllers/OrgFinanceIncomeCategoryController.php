...
class OrgFinanceIncomeCategoryController extends Controller
{
    use OrgFinanceScope;
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'code' => [
                ...
                'string',
                'max:50',
                Rule::unique('income_categories')->where(fn ($q) => $q->where('organization_id', $orgId)->whereNull('school_id')->whereNull('deleted_at')),
            ],
    ...
    public function update(Request $request, string $id)
    {
        ...
        $validated = $request->validate([
            ...
            'code' => [
                ...
                'string',
                'max:50',
                Rule::unique('income_categories')->where(fn ($q) => $q->where('organization_id', $orgId)->whereNull('school_id')->whereNull('deleted_at'))->ignore($id),
            ],
            'description' => 'nullable|string',
            'is_active' => 'nullable|boolean',
            'display_order' => 'nullable|integer|min:0',
