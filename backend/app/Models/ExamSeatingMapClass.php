...
class ExamSeatingMapClass extends Model
{
    use HasFactory, SoftDeletes;

    protected $connection = 'pgsql';

    protected $table = 'exam_seating_map_classes';
    ...
    protected static function boot(): void
    {
        ...
        static::creating(function (ExamSeatingMapClass $mapClass): void {
            if (empty($mapClass->id)) {
                $mapClass->id = (string) Str::uuid();
            }
        });

        static::saving(function (ExamSeatingMapClass $mapClass): void {
            $mapExamId = ExamSeatingMap::query()
                ->whereKey($mapClass->exam_seating_map_id)
                ->where('organization_id', $mapClass->organization_id)
                ->where('school_id', $mapClass->school_id)
                ->value('exam_id');
            $classExamId = ExamClass::query()
                ->whereKey($mapClass->exam_class_id)
                ->where('organization_id', $mapClass->organization_id)
                ->where('school_id', $mapClass->school_id)
                ->value('exam_id');
