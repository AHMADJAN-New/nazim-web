...
class AcademicYearSeeder extends Seeder
{
     ...
     *
     * Creates 2 academic years for each organization:
    ...
    public function run(): void
    {
        ...
        foreach ($organizations as $organization) {
            $this->command->info("Creating academic years for {$organization->name}...");

            // Create 2 academic years for this organization
            $created = $this->createAcademicYearsForOrganization($organization->id);
            $totalCreated += $created;

            $this->command->info("  → Created {$created} academic year(s) for {$organization->name}");
        }
    ...
    /**
     * Create academic years for a specific organization
     */
    protected function createAcademicYearsForOrganization(string $organizationId): int
    {
        $createdCount = 0;
    ...
    protected function createAcademicYear(
        ...
        // Check if academic year already exists for this organization
        ...
        if ($existing) {
            $this->command->info("  ✓ Academic year '{$name}' already exists for organization.");
            
            // Update is_current if needed (only if it's supposed to be current)
            if ($isCurrent && !$existing->is_current) {
                // Unset other current years for this organization first
        ...
        // If this is the current year, unset other current years for this organization first
        if ($isCurrent) {
            AcademicYear::where('organization_id', $organizationId)
                ->whereNull('deleted_at')
                ->update(['is_current' => false]);
