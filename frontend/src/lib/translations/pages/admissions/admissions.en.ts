/**
 * admissions page translations (en).
 * Auto-generated by: scripts/i18n/migrate-to-page-structure.ts
 */

export default {
  admissions: {
    academicYear: "Academic Year",
    academicYearBreakdown: "By academic year",
    acrossAllResidencyTypes: "Across all residency types",
    actions: "Actions",
    active: "Active",
    activeStudents: "Active students",
    add: "Admit Student",
    additionalInfo: "Additional",
    admissionDateTooLong: "Admission date is too long",
    admissionDetails: "Admission Details",
    admissionInformation: "Admission Information",
    admissionNumber: "Admission #",
    admissionYear: "Admission Year",
    admissionYearMaxLength: "Admission year must be 10 characters or less",
    admitStudent: "Admit student",
    admitStudentFromRegistration: "Admit student from registration",
    admitted: "Admitted",
    allResidency: "All Residency",
    assignRoom: "Assign room",
    awaitingActivation: "Awaiting activation",
    basicInfo: "Basic Info",
    boarder: "Boarder",
    boarderNo: "Day scholar",
    boarderOnly: "Boarders only",
    boarderOnlyHint: "Show only boarding students",
    boarderYes: "Boarder",
    bulkDeactivate: "Bulk deactivate",
    bulkDeactivateDescription: "Bulk deactivate description",
    bulkDeactivateTitle: "Deactivate Student Admissions",
    chooseStudent: "Choose student",
    classPlacement: "Class placement",
    homeroomTeacher: "Class teacher",
    homeroomRoom: "Classroom",
    sectionNotes: "Section notes",
    allAdmissionsForStudent: "All admissions for this student",
    selectAdmissionHint: "Select another admission to view it in this panel.",
    class: "Class",
    contact: "Contact",
    currentlyStudying: "Currently studying",
    deactivate: "Deactivate",
    dialogDescription: "Map a registered learner into a class, academic year, and residency type with status tracking.",
    enrollmentStatus: "Enrollment Status",
    enrollmentType: "Enrollment Type",
    enrollmentTypePlaceholder: "Boarder / Day scholar",
    enrollmentTypeTooLong: "Enrollment type is too long",
    exportFailed: "Failed to generate PDF report",
    feeStatus: "Fee Status",
    feeStatusPlaceholder: "Paid / Partial / Waived",
    feeStatusTooLong: "Fee status is too long",
    filtersTitle: "Filters",
    graduated: "Graduated",
    guardian: "Guardian",
    guardianPhone: "Guardian phone",
    inactive: "Inactive",
    invalidDateRangeDescription: "The start date must be before the end date.",
    invalidDateRangeTitle: "Invalid date range",
    limitReached: "Student limit reached. Please disable an old admission record first or upgrade your plan to add new admissions.",
    classCapacityReachedMessage: "This class has reached capacity. Please increase class limit before admitting a new student.",
    goToClassesSettings: "Go to Classes",
    classFull: "Class full",
    latest: "Latest",
    unassignedClass: "Not assigned to a class",
    list: "Admissions",
    listDescription: "Overview of class placements and residency tracking.",
    noClassesAssignedToYear: "No classes have been assigned to this academic year yet",
    noClassesForYear: "No classes available for this academic year",
    noDataFound: "No data found.",
    noRoomsFound: "No rooms found.",
    noStudentsFound: "No students found.",
    noStudentsAvailable: "No available students. All students may already be admitted.",
    notRequired: "Not required",
    pending: "Pending",
    pendingAdmitted: "Pending/Admitted",
    placementColumn: "Placement",
    placementCurrent: "Current",
    placementClosed: "Closed",
    placementHistory: "History",
    placementInClass: "In class",
    placementNeedsClass: "Needs class",
    placementOrphaned: "Section removed — reassign",
    openInList: "Open in list",
    placementFilterAll: "All placement",
    placementFilterPlaced: "In class",
    placementFilterNeedsClass: "Needs class",
    placementFilterOrphaned: "Invalid section",
    placedInClass: "Placed in class",
    placedInClassDescription: "Active admissions with a valid class section",
    needsClassAssignment: "Needs class",
    needsClassDescription: "Active admissions without a valid class section",
    needsClassWithOrphaned: "Includes {count} with removed sections",
    placementNotCurrent: "Not current",
    bulkStatusTitle: "Bulk Admission Status",
    bulkStatusDescription:
      "Update the admission status for {count} selected record(s). The linked student status will be synchronized automatically.",
    bulkStatusUpdate: "Update Status",
    bulkStatusAction: "Bulk Status ({count})",
    bulkAssignFromStudentsTitle: "Bulk assign to class",
    bulkAssignFromStudentsDescription:
      "Assign the same class section, residency, and optional hostel room to {count} selected student(s). Each student must already have an admission for the academic year of the chosen section.",
    bulkAssignPreviewHeading: "Selected students",
    bulkAssignPreviewMore: "…and {count} more",
    bulkAssignOnlyWithoutClass: "Only students still without a class section",
    bulkAssignOnlyWithoutClassHint:
      "When enabled, students who already have a class for that academic year are skipped. Turn off to update their placement too.",
    bulkAssignRoomHint: "Optional shared hostel room for all boarders in this batch. Leave empty if rooms are assigned later.",
    bulkAssignKeepStatus: "Keep current enrollment status",
    bulkAssignSubmit: "Apply placement",
    bulkAssignPlacementSuccess: "Placement updated for {updated} student(s).",
    bulkAssignPlacementPartial: "Updated {updated} student(s); others were skipped or could not be resolved.",
    bulkAssignPlacementNoop: "No rows changed ({skipped} already matched).",
    bulkAssignPlacementFailed: "Bulk placement failed",
    bulkAssignPlacementNoneResolved:
      "No placements applied ({count} student(s)). They may already have a class for this year—turn off \"Only students still without a class section\" to move them—or have no admission for this academic year.",
    placementNotes: "Placement Notes",
    quick: "Quick",
    quickMode: "Quick mode",
    placementNotesMaxLength: "Notes must be 500 characters or less",
    placementNotesPlaceholder: "Health, guardian approvals, or special considerations",
    recentAdmissions: "Recent admissions",
    records: "records",
    removeAdmission: "Remove admission?",
    removeAdmissionDescription: "This will keep the student registration but remove their class placement record.",
    reportExported: "PDF report generated successfully",
    reportFilters: "Report filters",
    reportSubtitle: "Analyze admissions performance across schools and years.",
    reportTitle: "Admissions Report",
    resetFilters: "Reset filters",
    residency: "Residency",
    residencyBreakdown: "Residency breakdown",
    residencyInfo: "Residency",
    residencyType: "Residency Type",
    room: "Room",
    roomDisabledWhenNotBoarder: "Room is only required for boarders.",
    roomDorm: "Room / Dorm",
    school: "School",
    schoolBreakdown: "By school",
    schoolClass: "School / Class",
    schoolRequiredForExport: "A school is required to export the report.",
    schoolSwitched: "School switched successfully",
    searchRoom: "Search rooms...",
    searchStudent: "Search by student name, admission number, or card number",
    section: "Section",
    selectAcademicYear: "Select academic year",
    shortcutsHint: "Shortcuts: Esc = close, Ctrl/Cmd+Enter = save",
    startOver: "Start over",
    selectAcademicYearFirst: "Select academic year first",
    selectAcademicYearToSeeClasses: "Select an academic year above to see available classes",
    selectClass: "Select class",
    selectClassSection: "Select class & section",
    selectResidency: "Select residency",
    selectSchool: "Select school",
    shift: "Shift",
    shiftPlaceholder: "Morning / Evening",
    shiftTooLong: "Shift is too long",
    status: "Status",
    statusBreakdown: "Status breakdown",
    student: "Student",
    studentFromRegistration: "Student (from registration)",
    studentsWithAccommodation: "Students with accommodation",
    subtitle: "Admit registered students into classes with residency and year tracking.",
    summary: "Summary",
    suspended: "Suspended",
    title: "Student Admissions",
    totalAdmissions: "Total admissions",
    totalAdmissionsLabel: "Total admissions",
    updateAdmission: "Update admission",
    withdrawn: "Withdrawn",
    year: "Year"
  }
};
