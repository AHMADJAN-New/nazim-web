...
export const csp = {
  generate: (options: {
    ...
    // Build connect-src with Supabase URLs and Laravel API
    ...
    // Build img-src and frame-src with Supabase URL
...
export const initializeSecurity = () => {
  ...
  if (import.meta.env.DEV) {
    ...
    // In development, allow HTTP connections to localhost/127.0.0.1 for local Supabase and Laravel API
    ...
    // Add Supabase URL for images and frames in development
    if (supabaseUrl) {
      try {
        const url = new URL(supabaseUrl);
        imgSrcParts.push(url.origin);
