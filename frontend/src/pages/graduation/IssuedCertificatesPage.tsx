...
export default function IssuedCertificatesPage() {
  ...
  const reportColumns = useMemo(() => [
    ...
    { key: 'batch_graduation_date', label: t('events.graduationDate') || 'Graduation Date' },
    { key: 'batch_class', label: t('search.class') || 'Class' },
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <PageHeader
        title={t('status.issued') ?? 'Issued Certificates'}
        ...
        rightSlot={
          <ReportExportButtons
            ...
            title={t('status.issued') || 'Issued Certificates Report'}
      ...
      <FilterPanel title={t('events.filters') || 'Search & Filter'}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <Input value={batchId || ''} onChange={(e) => setBatchId(e.target.value || undefined)} placeholder={t('events.autoGenerated')} />
          </div>
          <div>
            <Label>{t('events.name') ?? 'Student ID'}</Label>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('status.issued') ?? 'Issued Certificates'}</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => handleDownload(cert.id)}>
                            ...
                            {t('events.download') || 'Download PDF'}
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem asChild>
                            ...
                            >
                              ...
                              {t('events.verify') || 'Verify Certificate'}
                            </a>
                          </DropdownMenuItem>
                          {!cert.revoked_at && (
                            <>
